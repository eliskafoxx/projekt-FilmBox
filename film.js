...
filmy.map((film)=>{
	...
	const hvezda = document.querySelectorAll(".fa-star")
let hvezdaPocitadlo
const hvezdaZvyrazneni= (e) => {
	let hvezdaPocet
	hvezdaPocet = e.target.textContent
	if (e.type === "click") {
		hvezdaPocitadlo = hvezdaPocet
	}
	hvezda.forEach((hvezda, index) => {
		if (e.type === "mouseleave") {
			hvezdaPocet = hvezdaPocitadlo
		}
		if (index < hvezdaPocet) {
...
hvezda.forEach((hvezda) => {
	hvezda.addEventListener("click", hvezdaZvyrazneni)
	hvezda.addEventListener("mouseenter", hvezdaZvyrazneni)
	hvezda.addEventListener("mouseleave", hvezdaZvyrazneni)
})
})